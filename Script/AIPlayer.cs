...
public class AIPlayer : MonoBehaviour
{
    ...
    private Vector3 sideToCheck; // Tells which side to check when making sure the player is not too close to an object
    private Vector3 whereToGrab; // Tells where to grab if it was determined the player is too close to the object
    ...
    /// <summary>
    /// Checks if the player is right next to an object in the direction they want to travel, if they are will try to climb up to get over the object
    /// </summary>
    bool checkIfTooClose()
    {
        if (gateTotheRight)
        {
            sideToCheck = new Vector3(1, 0, 0);
            whereToGrab = new Vector3(0.1f, 1, 0);
        }
        ...
        if (Physics.Raycast(playerBody.transform.position, playerBody.transform.TransformDirection(sideToCheck), out hit, maxArmDistance))
        {
            if (hit.collider.tag == "Climbable" && hit.distance <= 0.2f)
            {
                if (Physics.Raycast(playerBody.transform.position, playerBody.transform.TransformDirection(whereToGrab), out hit, maxArmDistance))
                {
    ...
    /// <summary>
    /// Tries raycasts in order, those of a higher order are considered to be weighted more highly.
    /// </summary>
    ...
    /// <summary>
    /// While running sends a left click with a hold time defined by "holdTime". This is the main function of the AI. 
    /// </summary>
    /// <returns></returns>
